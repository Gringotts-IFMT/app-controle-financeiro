...
class DatabaseService {
  ...
  Future<void> saveUserProfile(String userId, Map<String, dynamic> userData) async {
    try {
      await _firestore.collection(_usersCollection).doc(userId).set(userData, SetOptions(merge: true));
    } catch (e) {
  ...
  Future<void> addTransaction(FinancialTransaction transaction, String userId) async {
    try {
      // CORREÇÃO AQUI: Comparar transaction.userId com userId passado
      if (transaction.userId != userId) {
        throw Exception("DatabaseService: Inconsistência de UserId na transação ao adicionar.");
      }
      await _firestore.collection(_transactionsCollection).add(transaction.toMap());
    } catch (e) {
  ...
  Future<List<FinancialTransaction>> getAllTransactions(String userId) async {
    try {
      final querySnapshot = await _firestore
          ...
          .orderBy('date', descending: true) // Ordena pela data, mais recente primeiro
          .get();
  ...
  Stream<List<FinancialTransaction>> getTransactions(bool isExpense, String userId) {
    return _firestore
        .collection(_transactionsCollection)
        .where('userId', isEqualTo: userId) // Filtra pelo userId
  ...
  Future<void> deleteTransaction(String transactionId, String userId) async {
    try {
      final doc = await _firestore.collection(_transactionsCollection).doc(transactionId).get();
      if (!doc.exists) { 
        print('DatabaseService: Transação com ID $transactionId não encontrada para exclusão.');
        return; 
      }
      if (doc.data()?['userId'] != userId) { 
        throw Exception("DatabaseService: Você não tem permissão para deletar esta transação.");
      }
  ...
  Future<void> addOrUpdateMetaEconomia(MetaEconomia meta, String userId) async {
    try {
      if (meta.userId != userId) {
        throw Exception("DatabaseService: Inconsistência de UserId na meta ao adicionar/atualizar.");
      }
      if (meta.id == null || meta.id!.isEmpty) {
        ...
        final doc = await _firestore.collection(_metasEconomiaCollection).doc(meta.id).get();
        if (!doc.exists) {
          print('DatabaseService: Meta com ID ${meta.id} não encontrada para atualização.');
          throw Exception("Meta não encontrada para atualização.");
        }
        if (doc.data()?['userId'] != userId) {
          throw Exception("DatabaseService: Você não tem permissão para atualizar esta meta.");
        }
        await _firestore.collection(_metasEconomiaCollection).doc(meta.id).update(meta.toMap());
      }
    } catch (e) {
      print('DatabaseService: Erro ao adicionar/atualizar meta de economia: $e');
      rethrow;
    }
  }
  ...
  Future<void> deleteMetaEconomia(String metaId, String userId) async {
    try {
      final doc = await _firestore.collection(_metasEconomiaCollection).doc(metaId).get();
      if (!doc.exists) {
        print('DatabaseService: Meta com ID $metaId não encontrada para exclusão.');
        return;
      }
      if (doc.data()?['userId'] != userId) {
        throw Exception("DatabaseService: Você não tem permissão para deletar esta meta.");
      }
      await _firestore.collection(_metasEconomiaCollection).doc(metaId).delete();
    } catch (e) {
      print('DatabaseService: Erro ao deletar meta de economia no Firebase: $e');
      rethrow;
    }
  }
  ...
  Future<bool> atualizarValorAtualMeta(String id, double novoValor, String userId) async {
    try {
      final doc = await _firestore.collection(_metasEconomiaCollection).doc(id).get();
      if (!doc.exists) return false;
      if (doc.data()?['userId'] != userId) {
        throw Exception("DatabaseService: Acesso não autorizado para atualizar valor da meta.");
      }
  ...
  Future<bool> atualizarStatusMeta(String id, String novoStatus, String userId) async {
    try {
      final doc = await _firestore.collection(_metasEconomiaCollection).doc(id).get();
      if (!doc.exists) return false;
      if (doc.data()?['userId'] != userId) {
        throw Exception("DatabaseService: Acesso não autorizado para atualizar status da meta.");
      }
      await _firestore.collection(_metasEconomiaCollection).doc(id).update({
        'status': novoStatus,
        'dataAtualizacao': Timestamp.now(),
  ...
  Future<List<MetaEconomia>> buscarMetasVencendo(int diasParaVencer, String userId) async {
    try {
      ...
      return querySnapshot.docs.map((doc) => MetaEconomia.fromMap(doc.data() as Map<String, dynamic>, doc.id)).toList();
    } catch (e) {
